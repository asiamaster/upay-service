...
public class TradeSentinel extends Sentinel {
    ...
    public void override(TradeSentinel sentinel) {
    ...
    void checkPassport(Passport passport) {
        if (!Permission.hasPermission(passport.getPermission(), Permission.FOR_TRADE)) {
            throw new RiskControlException(ErrorCode.OPERATION_NOT_ALLOWED, "风控提示：用户账号无交易权限");
        }
        // 提现金额不能超过单笔提现限额
